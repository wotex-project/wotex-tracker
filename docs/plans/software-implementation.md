...
WTR.12's completion axes control every phase. Finish repository-owned development
and local acceptance with fixtures, deterministic peers, emulators, QEMU,
containers and loopback services before scheduling physical/provider work. A
missing device, SIM, carrier, public-provider credential, Apple membership or
registry entitlement never blocks code that can be implemented locally. Record
that external work only on the qualification or distribution axis.

This is a greenfield codebase. Apply WTR.13's Zig boundary while implementing:
replace the standalone Rust protocol consumer with Zig, emit generated C-ABI
registration tables as Zig, and consolidate duplicated Zig mobile build logic.
Keep typed Apple-framework lifecycle code in Objective-C/Swift and do not rewrite
Elixir/OTP orchestration, SQL or POSIX launchers merely for language uniformity.
...
## Phase 1 — observation, resolution and reference-fixture decoder
...
Ruuvi RAWv2 is retained only as deterministic software regression data; no
RuuviTag is a purchase, product PoC or hardware qualification target.
...
This Ruuvi packet is legacy software-fixture evidence, not the selected product
topology. Replace it with a deterministic TAT140 IMEI/Codec-8-Extended TCP peer,
ACK assertion and retained tracking-state check before marking Pi development
complete. The local peer requires no physical device, SIM or carrier.
...
independent Zig HTTP consumer. Older Rust receipts are retained as historical
evidence; current host, Linux ARM64 and integrated receipts are generated by the
Zig consumer. The lane
performs an actual service restart, network
