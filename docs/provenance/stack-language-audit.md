...
`wotex*` sibling checkouts on 2026-09-16, then refreshed tracked-file counts
after concurrent sibling work. Counts include `.py` files and Python
...
| `wotex-coap` | 0 | The former `test/interop/libcoap/run.py` runner has been removed in the sibling checkout. `docs/provenance/software-udp-dtls-v1.json` still names it as the historical runner. | Update the stale provenance reference in the sibling repository; no active Python build/test dependency remains in the inspected paths. |
...
`wotex-modbus`, `wotex-nx` and `wotex-runtime`. There are 34 tracked Python
files/manifests in the four sibling repositories that still contain them. A
