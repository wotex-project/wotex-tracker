defmodule Wotex.Tracker.Service.RuleTransition do
  ...
  alias Wotex.Tracker.TransportDegradation
  alias Wotex.Tracker.TransportDegradation.State
  ...
  @spec new(String.t(), State.t() | nil, map()) :: {:ok, t()} | {:error, atom()}
  def new(scope, previous, result) do
    with true <- Codec.id?(scope),
         ...
         :ok <- optional_event(result["event"]),
         record_id = @kind <> ":" <> state.policy.id,
  ...
  def validate(%__MODULE__{} = value) do
    with true <- value.kind == @kind,
         ...
         {:ok, state} <- TransportDegradation.state_from_map(value.document),
         ...
         :ok <- optional_event(value.event),
         true <- event_matches?(value.event, state, value.mode, value.action),
  ...
  defp optional_event(nil), do: :ok
  defp optional_event(event), do: TransportDegradation.validate_event(event)

  defp event_matches?(nil, _state, "live", "none"), do: true
  defp event_matches?(nil, _state, "replay", "prohibited"), do: true

  defp event_matches?(event, state, mode, action) when is_map(event) do
    expected_action =
