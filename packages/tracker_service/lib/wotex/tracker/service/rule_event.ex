defmodule Wotex.Tracker.Service.RuleEvent do
  ...
  """
  alias Wotex.Tracker.{Geofence, GeofenceCrossing, PositionSample}
  alias Wotex.Tracker.Service.Codec

  @crossing_kind "geofence_crossing"
  @input_fields ~w(schema fence from to policy)
  @fields [
    :scope,
    :kind,
    :rule_id,
  ...
  def validate(%__MODULE__{kind: @crossing_kind} = value) do
    with true <- Codec.id?(value.scope) and Codec.id?(value.rule_id),
         true <- exact_fields?(value.inputs, @input_fields),
         ...
         true <- event_matches?(value.event, fence, from, to, policy),
  ...
  defp event_matches?(event, fence, from, to, policy) do
