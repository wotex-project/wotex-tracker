defmodule Wotex.Tracker.Service.Cursor do
  ...
  defp data?(
    ...
    kind in ~w(observations resolutions evidence enrollments things state policies saved_queries rules alerts operations access_audit arming owner_presence) and
  ...
  defp data?(
    ...
    kind == "thing_alerts" and Codec.id?(thing) and
      data?(%{
        "kind" => "alerts",
        "generation" => generation,
        "after" => after_id,
  ...
  defp data?(
    ...
    Codec.id?(thing) and Codec.time?(from_at) and Codec.time?(to_at) and from_at < to_at and
      data?(%{
        "kind" => "alerts",
        "generation" => generation,
        "after" => after_id,
