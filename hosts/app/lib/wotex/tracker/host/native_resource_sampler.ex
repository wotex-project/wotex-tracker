defmodule Wotex.Tracker.Host.NativeResourceSampler do
  ...
  The production source reads only fixed procfs paths. Sampling failure is
  isolated from the service and produces no partial or synthetic metric.
  """

  use GenServer

  alias Wotex.Tracker.Host.LinuxProcfs
  ...
  @doc false
  def default_source({:unix, :linux}), do: {LinuxProcfs, &File.read/1}
  ...
  def handle_info(:sample, state) do
    case call(state.source) do
      {:ok, measurements} ->
        OperationalTelemetry.native_resource_sample(:service, :linux_procfs, measurements)
