// build_graph.zig — Shared native build orchestration for WotexTrackerMobile.
//
// This graph owns the common native compile and link pipeline for both the
// iOS simulator and physical-device wrappers. Platform-specific SDK, target,
// deployment, and device-runtime inputs are selected through Flavor.
//
//   Plain C (zig cc):
...
pub const Flavor = enum { simulator, device };

const Platform = struct {
    sdk: []const u8,
    swift_target: []const u8,
    arch: []const u8,
    minimum_flag: []const u8,
    label: []const u8,
};

fn platform(comptime flavor: Flavor) Platform {
    return switch (flavor) {
        .simulator => .{
            .sdk = "iphonesimulator",
            .swift_target = "arm64-apple-ios17.0-simulator",
            .arch = "arm64",
            .minimum_flag = "-mios-simulator-version-min=17.0",
            .label = "simulator",
        },
        .device => .{
            .sdk = "iphoneos",
            ...
            .arch = "arm64",
            .minimum_flag = "-miphoneos-version-min=17.0",
            .label = "device",
        },
    };
}

pub fn build(b: *std.Build, comptime flavor: Flavor) void {
    const ios = platform(flavor);
    const target = switch (flavor) {
        .simulator => b.resolveTargetQuery(.{
            .cpu_arch = .aarch64,
            .os_tag = .ios,
            .abi = .simulator,
        }),
        .device => b.resolveTargetQuery(.{
            .cpu_arch = .aarch64,
            .os_tag = .ios,
        }),
    };
    ...
    const otp_release = if (flavor == .device) required(b, "otp_release", "OTP release number (e.g. 29)") else "";
    const sdkroot = required(b, "sdkroot", b.fmt("iOS {s} SDK path", .{ios.label}));
    ...
    const sqlite_static = if (flavor == .device)
        b.option(bool, "sqlite_static", "exqlite NIF is statically linked (-DMOB_STATIC_SQLITE_NIF on driver_tab)") orelse false
    else
        false;
    const epmd_build_src = if (flavor == .device) required(b, "epmd_build_src", "OTP tree exposing erts/epmd/src + erts/include") else "";
    const errno_compat = if (flavor == .device) required(b, "errno_compat", "Absolute path to the generated erl_errno_id_compat.c shim") else "";
    const sqlite_static_lib = if (flavor == .device)
        b.option([]const u8, "sqlite_static_lib", "Absolute path to sqlite3_nif.a (when exqlite is statically linked)") orelse ""
    else
        "";
    ...
    const objects_step = b.step("objects", b.fmt("Compile C, ObjC, and Swift objects for iOS {s}", .{ios.label}));
    const binary_step = b.step("binary", b.fmt("Compile and link the iOS {s} binary", .{ios.label}));
    ...
    // --- ObjC via xcrun cc -----------------------------------------------------
    const mob_beam_flags: []const []const u8 = if (flavor == .device)
        &.{
            "-DMOB_BUNDLE_OTP",
